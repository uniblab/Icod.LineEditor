...
function Get-ExecutableProjects {
    ...
    foreach ($projectPath in $ProjectPaths) {
        ...
        if ($outputType -in @('Exe', 'WinExe')) {
            ...
            if ([string]::IsNullOrWhiteSpace($assemblyName)) { $assemblyName = [System.IO.Path]::GetFileNameWithoutExtension($projectPath) }
...
function Get-PackageMetadata {
    ...
    try {
        $nuspec = @($archive.Entries | Where-Object { $_.FullName.EndsWith('.nuspec', [System.StringComparison]::OrdinalIgnoreCase) })
        if (1 -ne $nuspec.Count) { throw "Package '$PackagePath' must contain exactly one nuspec." }
        ...
        return [pscustomobject]@{
            ...
            Readme = if ($null -eq $metadata.SelectSingleNode("*[local-name()='readme']")) { '' } else { $metadata.SelectSingleNode("*[local-name()='readme']").InnerText.Trim().Replace('\\','/') }
        }
